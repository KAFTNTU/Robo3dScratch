...
`;
  ...
  function createGearMesh(THREE, {teeth=16, outerR=0.48, innerR=0.40, thickness=0.06, holeR=0.09}={}){
    ...
    const mat = new THREE.MeshStandardMaterial({color: 0x222222, roughness: 0.65, metalness: 0.1});
  ...
  function buildCar(THREE, opts, textures){
    ...
    // cover (textured)
    const fallback = new THREE.MeshStandardMaterial({ color: 0x2dd4bf, roughness: 0.5, metalness: 0.05 });
    ...
    function addGear(x, y, z, idxInSide, sideSign){
      ...
      const mesh = createGearMesh(THREE, {teeth: 16, outerR: gearOuterR, innerR: gearInnerR, thickness: 0.05, holeR: 0.06});
  ...
  async function start(userOpts){
    ...
    const grid = new THREE.GridHelper(2000, 200, 0xcbd5e1, 0x1f2937);
    grid.position.y = 0;
    scene.add(grid);
    ...
    function syncTrack3D(){
      ...
      // Switch world look depending on track type:
      // - Line tracks: light-gray background like a clean paper sheet.
      // - Others: dark workshop.
      const isLineTrack = (tr.kind === 'line');
      scene.background = isLineTrack ? bgLight : bgDark;
      if (groundMat){
        const isSandbox = ((sim.trackName||'') === 'Sandbox');
        // Light gray "paper" for line tracks; checker tiles for sandbox; dark for everything else.
        groundMat.color.setHex(isLineTrack ? 0xf3f4f6 : (isSandbox ? 0xffffff : 0x111827));
        groundMat.map = isSandbox ? sandboxTex : null;
      ...
      // Make the 3D line much thicker so it's clearly visible in 3D mode.
      const lw = (tr.lineWidth||16) * k * 5;
      const outerR = Math.max(0.01, lw*0.60);
      const innerR = Math.max(0.008, lw*0.40);

      const matOuter = new THREE.MeshStandardMaterial({ color: 0x050505, roughness: 0.95, metalness: 0.0, transparent:true, opacity:0.65 });
      const matMain  = new THREE.MeshStandardMaterial({ color: 0x111827, roughness: 0.9, metalness: 0.0, transparent:true, opacity:0.95 });
      ...
      if (tr.kind==='arena'){
        ...
        (tr.walls||[]).forEach((seg)=>{
          ...
          const geo = new THREE.BoxGeometry(len, wallH, innerR*2);
        ...
        if (pts.length >= 2){
          ...
          const v = src.map(p=> new THREE.Vector3((p[0]||0)*k, 0.01, (p[1]||0)*k));
          const closed = ((sim.trackName||'') !== 'CustomLine');
          const curve = new THREE.CatmullRomCurve3(v, closed);
          const segments = Math.min(1000, Math.max(80, v.length*10));
          const geoOuter = new THREE.TubeGeometry(curve, segments, outerR, 12, true);
          const geoMain  = new THREE.TubeGeometry(curve, segments, innerR, 12, true);
          trackGroup.add(new THREE.Mesh(geoOuter, matOuter));
          trackGroup.add(new THREE.Mesh(geoMain, matMain));
        }
      }
    }
    ...
    canvas3d.addEventListener('pointerdown', (e)=>{
      ...
      if (e.button === 0 && rDown && pointerHitsRobot(e.clientX, e.clientY)){
        e.preventDefault();
        e.stopPropagation();
      ...
      if (e.button === 0 && spaceDown && pointerHitsRobot(e.clientX, e.clientY)){
        e.preventDefault();
        e.stopPropagation();
    ...
    if (Controls){
      ...
      if (typeof orbit.panSpeed === 'number') orbit.panSpeed = -0.9;
    }
    ...
    canvas3d.addEventListener('pointerdown', (e)=>{
      ...
      const idx = pickObstacle(e.clientX, e.clientY);
      if (idx < 0) return;
      e.preventDefault();
      e.stopPropagation();
      obsEdit.active = true;
    ...
    function update(){
      ...
      const {x,y,a} = getPose();
      const k = 1/opts.pxPerUnit;
      car.position.set(x*k, 0.29, y*k);
      // 2D: a=0 is "up"; in 3D, forward is -Z for our car
